...
func (scraper *kucoinScraper) watchConfig(ctx context.Context, lock *sync.RWMutex) {
	...
	envKey := strings.ToUpper(KUCOIN_EXCHANGE) + "_WATCH_CONFIG_INTERVAL"
	interval, err := strconv.Atoi(utils.Getenv(envKey, "3600"))
	if err != nil {
		log.Errorf("KuCoin - Failed to parse %s: %v.", envKey, err)
		return
	}
	...
	last, err := models.GetExchangePairMap(KUCOIN_EXCHANGE)
	if err != nil {
	...
	for {
		select {
		case <-ticker.C:
			current, err := models.GetExchangePairMap(KUCOIN_EXCHANGE)
			if err != nil {
				log.Errorf("KuCoin - GetExchangePairMap: %v.", err)
				continue
			}
...
func (scraper *kucoinScraper) applyConfigDiff(ctx context.Context, lock *sync.RWMutex, last map[string]int64, current map[string]int64) {
	...
	for _, p := range added {
		...
		if err != nil {
			log.Errorf("KuCoin - Failed to subscribe to %s: %v", ep.ForeignName, err)
