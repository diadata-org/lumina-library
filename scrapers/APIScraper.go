...
func RunScraper(
	...
	case BITSTAMP_EXCHANGE:
		ctx, cancel := context.WithCancel(context.Background())
		scraper := NewBitstampScraper(ctx, pairs, branchMarketConfig, wg)
		watchdogDelay, err := strconv.Atoi(utils.Getenv("BITSTAMP_WATCHDOG", "300"))
		if err != nil {
			log.Errorf("parse BITSTAMP_WATCHDOG: %v.", err)
		}
		watchdogTicker := time.NewTicker(time.Duration(watchdogDelay) * time.Second)
		lastTradeTime := time.Now()
		for {
			...
			case <-watchdogTicker.C:
				...
				if duration > time.Duration(watchdogDelay)*time.Second {
					err := scraper.Close(cancel)
					if err != nil {
						log.Errorf("Bitstamp - Close(): %v.", err)
					}
					log.Warnf("Closed Bitstamp scraper as duration since last trade is %v.", duration)
					failoverChannel <- BITSTAMP_EXCHANGE
					return
				}
			}
		}
	...
	case CURVE_BASE_EXCHANGE:
		NewCurveScraper(ctx, exchange, Exchanges[exchange].Blockchain, pools, tradesChannel, branchMarketConfig, wg)
	case AERODROMESLIPSTREAM_EXCHANGE:
		NewUniswapV3Scraper(ctx, exchange, Exchanges[exchange].Blockchain, pools, tradesChannel, branchMarketConfig, wg)
	case AERODROMEV1_EXCHANGE:
		NewAerodromeV1Scraper(ctx, exchange, Exchanges[exchange].Blockchain, pools, tradesChannel, branchMarketConfig, wg)
	}
}
